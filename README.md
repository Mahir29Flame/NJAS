...
On Windows the command is `python server.py`. Open **http://127.0.0.1:6729/stage.html** in Browser, allow the camera, and wave. That's the whole install: the server is stdlib Python, and the page loads its hand tracking (Google MediaPipe) and 3D (three.js) from CDNs on first run.

**Already in a Claude Code session with your agent?** One sentence does it all: *"clone https://github.com/Mahir29Flame/NJAS.git, then read NJAS/NJAS.md and set me up."* Your agent installs it, configures it, and wires itself in.
...
Copyright (c) 2026 Jared Rhodenizer (Original Author).
Modifications Copyright (c) 2026 Nahiyan Jarif Mahir.
